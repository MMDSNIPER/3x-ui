...
async function fetchAdmins() {
  loading.value = true;
  try {
    const msg = await HttpUtil.get('/panel/api/admins');
...
async function fetchInbounds() {
  const msg = await HttpUtil.get('/panel/api/inbounds/options');
...
async function doDelete(id) {
  const msg = await HttpUtil.delete(`/panel/api/admins/${id}`);
